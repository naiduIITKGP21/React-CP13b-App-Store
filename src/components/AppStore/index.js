...
class AppStore extends Component {
  ...
  onClickTab = tabId => {
    ...
    this.setState({activeTabId: tabId, searchInput})
  }

  onSearchInput = event => {
    const {activeTabId} = this.state
    this.setState({searchInput: event.target.value, activeTabId})
  }

  filterCategoryApps = tabId =>
    appsList.filter(eachApp => eachApp.category === tabId)

  render() {
    const {activeTabId, searchInput} = this.state
    const activeCategoryApps = this.filterCategoryApps(activeTabId)
    const searchedResultsFromActive = activeCategoryApps.filter(eachApp =>
      ...
      <div className="app-container">
        <div className="app-store">
          ...
          <ul className="apps-list">
            {searchedResultsFromActive.map(eachApp => (
